...
import { SelectEntry, TextFieldEntry, isTextFieldEntryEdited, ListGroup, isTextAreaEntryEdited, TextAreaEntry, HeaderButton, CheckboxEntry, isCheckboxEntryEdited, DescriptionEntry, Group } from '@bpmn-io/properties-panel';
import 'react';
import { useService } from 'bpmn-js-properties-panel';
import { jsx, Fragment, jsxs } from '@bpmn-io/properties-panel/preact/jsx-runtime';
...
IoPalette.prototype.getPaletteEntries = function () {
  ...
  function createListener(event, type) {
    let shape = elementFactory.createShape(assign({
      type: type
    }, {}));
  ...
  constructor(eventBus, bpmnFactory, bpmnUpdater) {
    ...
    this.execute(['shape.create'], HIGH_PRIORITY$5, function (event) {
      ...
      if (['bpmn:DataInput', 'bpmn:DataOutput'].includes(context.shape.type)) {
        ...
        if (type == 'bpmn:DataInput') {
          add(ioSpec.inputSets[0].get('dataInputRefs'), dataIO);
          add(ioSpec.get('dataInputs'), dataIO);
        } else {
          add(ioSpec.outputSets[0].get('dataOutputRefs'), dataIO);
          add(ioSpec.get('dataOutputs'), dataIO);
        }
      }
    });
    this.execute(['shape.delete'], HIGH_PRIORITY$5, function (event) {
      ...
      if (['bpmn:DataInput', 'bpmn:DataOutput'].includes(context.shape.type)) {
        ...
        let ioSpec = assureIOSpecificationExists(process, bpmnFactory);
        if (type == 'bpmn:DataInput') {
          remove(ioSpec.inputSets[0].get('dataInputRefs'), context.shape.businessObject);
          remove(ioSpec.get('dataInputs'), context.shape.businessObject);
        } else {
          remove(ioSpec.outputSets[0].get('dataOutputRefs'), context.shape.businessObject);
          remove(ioSpec.get('dataOutputs'), context.shape.businessObject);
        }
        if (context.shape.di.$parent) {
          remove(context.shape.di.$parent.planeElement, context.shape.di);
        }
  ...
  constructor(eventBus, bpmnFactory, commandStack, bpmnUpdater) {
    ...
    this.executed(['shape.delete'], HIGH_PRIORITY$4, function (event) {
      ...
      if (is(shape, 'bpmn:DataObjectReference') && shape.type !== 'label') {
        ...
        remove(flowElements, shape.businessObject);
        ...
        if (references.length === 0) {
          ...
          remove(dataFlowElements, dataObject);
        }
      }
    });
  }
...
function DataObjectPropertiesProvider(propertiesPanel, translate, moddle, commandStack, elementRegistry, modeling, bpmnFactory) {
  this.getGroups = function (element) {
    return function (groups) {
      ...
      if (isAny$1(element, ['bpmn:Process', 'bpmn:Participant']) || is(element, 'bpmn:SubProcess') && !element.collapsed) {
...
function ExtensionsPropertiesProvider(propertiesPanel, translate, moddle, commandStack, elementRegistry) {
  this.getGroups = function (element) {
    return function (groups) {
      ...
      } else if (isAny$1(element, ['bpmn:Task', 'bpmn:CallActivity', 'bpmn:SubProcess'])) {
      ...
      if (isAny$1(element, ['bpmn:ManualTask', 'bpmn:UserTask', 'bpmn:ServiceTask', 'bpmn:EndEvent', 'bpmn:ScriptTask', 'bpmn:IntermediateCatchEvent', 'bpmn:CallActivity', 'bpmn:SubProcess'])) {
        groups.push(createUserInstructionsGroup(element, translate, moddle, commandStack));
      }
      if (isAny$1(element, ['bpmn:ManualTask', 'bpmn:UserTask'])) {
        groups.push(createAllowGuestGroup(element, translate, moddle, commandStack));
      }
      if (is(element, 'bpmn:BoundaryEvent') && hasEventDefinition(element, 'bpmn:SignalEventDefinition') && isAny$1(element.businessObject.attachedToRef, ['bpmn:ManualTask', 'bpmn:UserTask'])) {
      ...
      if (isAny$1(element, ['bpmn:UserTask', 'bpmn:ManualTask'])) {
...
function isMessageElement(shapeElement) {
  return is(shapeElement, 'bpmn:SendTask') || is(shapeElement, 'bpmn:ReceiveTask') || isMessageEvent(shapeElement);
}
...
function canReceiveMessage(shapeElement) {
  ...
  if (isMessageEvent(shapeElement)) {
    return is(shapeElement, 'bpmn:StartEvent') || is(shapeElement, 'bpmn:CatchEvent');
  }
  return false;
}
...
function isCatchingEvent(element) {
  return isAny$1(element, ['bpmn:StartEvent', 'bpmn:IntermediateCatchEvent', 'bpmn:BoundaryEvent']);
}
function isThrowingEvent(element) {
  return isAny$1(element, ['bpmn:EndEvent', 'bpmn:IntermediateThrowEvent']);
}
function getConfigureGroupForType(eventDetails, label, includeCode, getSelect) {
  ...
  return function (props) {
    ...
    const boundaryCondition = is(element, 'bpmn:BoundaryEvent') && (hasEventDefinition(element, 'bpmn:ErrorEventDefinition') && element.businessObject.eventDefinitions[0].errorRef || hasEventDefinition(element, 'bpmn:EscalationEventDefinition') && element.businessObject.eventDefinitions[0].escalationRef || hasEventDefinition(element, 'bpmn:SignalEventDefinition') && element.businessObject.eventDefinitions[0].signalRef);
    if (includeCode) {
...
function isBpmnTask(element) {
  ...
  return is(element, 'bpmn:UserTask') || is(element, 'bpmn:ScriptTask') || is(element, 'bpmn:ServiceTask') || is(element, 'bpmn:SendTask') || is(element, 'bpmn:ReceiveTask') || is(element, 'bpmn:ManualTask');
}
...
function StandardLoopPropertiesProvider(propertiesPanel) {
  this.getGroups = function getGroupsCallback(element) {
    return function pushGroup(groups) {
      if ((is(element, 'bpmn:Task') || is(element, 'bpmn:CallActivity') || is(element, 'bpmn:SubProcess')) && typeof element.businessObject.loopCharacteristics !== 'undefined' && element.businessObject.loopCharacteristics.$type === 'bpmn:StandardLoopCharacteristics') {
        const group = {
  ...
  propertiesPanel.registerProvider(LOW_PRIORITY$2, this);
}
...
function hasPreAndPostScript(element) {
  return is(element, 'bpmn:Task') || is(element, 'bpmn:UserTask') || is(element, 'bpmn:ServiceTask') || is(element, 'bpmn:SendTask') || is(element, 'bpmn:ReceiveTask') || is(element, 'bpmn:ManualTask') || is(element, 'bpmn:CallActivity') || is(element, 'bpmn:BusinessRuleTask') || is(element, 'bpmn:SubProcess');
}
...
export { CallActivityPropertiesProvider, ConditionsPropertiesProvider, DataObjectPropertiesProvider, DataStorePropertiesProvider, ErrorPropertiesProvider, EscalationPropertiesProvider, ExtensionsPropertiesProvider, IoPropertiesProvider, MessagesPropertiesProvider, MultiInstancePropertiesProvider, SignalPropertiesProvider, StandardLoopPropertiesProvider, index as default, spiffworkflow as spiffModdleExtension, index as spiffworkflow };
//# sourceMappingURL=bpmn-js-spiffworkflow.esm.js.map
